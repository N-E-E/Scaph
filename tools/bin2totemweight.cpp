...
int main(int argc,char **argv)
{
    ...
    file = fopen(binfile,"rb");
    index_t* csr_wgh = new vertex_t[vert_count+1];
    ret = fread(csr_wgh,sizeof(vertex_t),edge_count,file);
    assert(ret == edge_count);
    fclose(file);
    
    sprintf(binfile,"%sweight.totem",filename);
    ofstream myout(binfile);
    myout<<"#Nodes: "<<vert_count<<endl;
    myout<<"#Edges: "<<edge_count<<endl;
    myout<<"#Undirected"<<endl;
    string buffer; 
    for(int index = 0; index < vert_count;index++){
        unsigned int bindex = csr_idx[index];
        unsigned int eindex = csr_idx[index+1];
        for(unsigned int lindex = bindex; lindex < eindex; lindex++){
            buffer += to_string(index) + "\t" + to_string(csr_ngh[lindex]) + "\t" + to_string(csr_wgh[lindex]) + "\n";
        }
    }
